...
public class AiChatController : ControllerBase
{
    ...
    public async Task<ActionResult<WebappAiConfigDto>> PutWebappConfig([FromBody] WebappAiConfigInput input)
    {
        ...
        // Voice-message Live model (null keeps existing; "" clears it so it inherits the call's Live model)
        // and the Live/TTS mode (normalized to "live"/"tts"; null keeps existing).
        if (input.VoiceLiveModel is not null) c.VoiceLiveModel = MergeText(input.VoiceLiveModel, c.VoiceLiveModel);
        if (input.VoiceMode is not null)
        {
