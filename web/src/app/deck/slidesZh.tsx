      ...
      <Slide>
        ...
        <div className="mt-[20px]">
          <Cols ratio="1fr 1fr" gap={48} align="start">
            <div className="space-y-[14px]">
              <Card title="通常的做法">
                录音。上传文件。转写。生成。合成。五段严格串行的跳转，而且模型从来没有听见你，它读到的是一份你的转写稿。
              </Card>
              <Bullets
                items={[
                  "EverVault 的实时通话和一次性语音消息，跑在同一个模型上，走 WebSocket。",
                  "你还在说的时候，声音就在往上传了。没有任何一步在等一份录完的文件。",
                  "所以你话音刚落，模型已经拿到了这一轮的大部分内容。它已经开始推理，甚至可能已经调用过一个工具。回复几乎立刻就开始。",
                ]}
              />
            </div>
            <div className="space-y-[14px]">
              <Card title="一次调用，两边的转写">
                语音和双方的文字都来自同一次流式调用，没有单独的转写、回复、合成三段。图片也走同一条路，所以对着一张照片说话依然只是一次调用。
              </Card>
              <Card title="结构上就不碰密钥">
                后端签发一个短期令牌，浏览器直接连上服务商。你的音频从不经过我的服务器，密钥也从不到达页面。一次普通的重连会刻意沿用同一个密钥，因为恢复句柄只在签发它的那个项目里有效；只有配额或鉴权导致的关闭才会切到池子里的下一个。
              </Card>
            </div>
          </Cols>
        </div>
        <div className="mt-[18px]">
          <Note>
            速度和深度是互相换的，而日常对话该买的就是速度。这也是为什么，别用嘴问它一道难的数学题。
          </Note>
        </div>
      </Slide>
    ),
