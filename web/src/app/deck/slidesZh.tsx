      ...
      <Slide>
        ...
        <div className="mt-[22px]">
          <Cols ratio="1.02fr 0.98fr" gap={48} align="start">
            ...
            <div className="space-y-[14px]">
              <Card title="这正是 AI 写出来的代码的样子">
                两种情况都一样能跑。到底有没有人真的看懂，在出问题之前是看不出来的；而出问题的那一刻，这个差别就是全部。
              </Card>
              <Bullets
                items={[
                  "用这些工具做了两年，确实有一段差距被抹平了：语法、API、样板代码、空白页面。这部分是真的被拉平了。",
                  "被拉大的是另一段：知道该要什么、知道什么时候答案是自信地错的、以及凌晨两点找出它为什么坏了。",
                  "这场分享里的每一个修复，都来自对底层的理解，而不是来自生成代码。自动播放的规则、回声的物理、召回的记忆该放在提示词的哪个位置。",
                ]}
              />
              <Note>
                这也正是为什么这里每一次失败都会把原始日志写进数据库。你没法对一个看不见的 bug 做推理。
              </Note>
            </div>
          </Cols>
        </div>
